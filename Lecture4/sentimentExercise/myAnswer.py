...
# Define a function to return an integer based on the sentiment of the input word
def sentimentScoreFunc(word):
...
# Run a function on each word, will return an RDD of just integers (basically a list of just integers)
word_map = all_word.map(sentimentScoreFunc)
# "long" way to write it, but does the exact same thing
# word_map = all_word.map(lambda s: sentimentScoreFunc(s))

# Reduce the list using the operator "add", which basically just adds each integer together
result = word_map.reduce(add)
# "long" way to write it, but does the exact same thing
# result = word_map.reduce(lambda a, b : a + b)
